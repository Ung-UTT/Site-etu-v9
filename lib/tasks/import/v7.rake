...
namespace :import do
  ...
  task :v7 do
    ...
    User.delete_all
    require "#{Rails.root}/db/seeds.rb" # create admin user
    ...
    results.each(:symbolize_keys => true) do |row|
      ...
      user = User.simple_create row[:login]
      user.email = (row[:email].blank? ? 'inconnu@example.com' : row[:email])
    ...
    results.each(:symbolize_keys => true) do |row|
      ...
      asso = Asso.create!(
        ...
        owner_id: Profile.all.map do |u|
          "#{u.firstname} #{u.lastname}"
        end.detect do |fullname|
          fullname == row[:nom_responsable]
